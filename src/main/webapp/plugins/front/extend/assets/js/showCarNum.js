...
//			alert("车牌："+$(".car_input").attr("data-pai"));
			select($(".car_input").attr("data-pai"));
			layer.closeAll();
		}else if(jj==37){
			if($(".ppHas").length==0){
...
function select(carNum){
	$('.list-cl').empty();
	$.ajax({
		url:'cYCarNumAll.shtml',
		data:{'carNum':carNum},
		type:'post',
		dataType:'json',
		success:function(obj){
			var carNumAll = obj.carNumAll;
			if(carNum!=''&&carNum!=undefined){
				$('#fh').html("<div class='btn btn-cz'><a href='#' onclick='javascript:window.location.reload();'>重置</a></div>");
			}else{
				$('#fh').html();
			}
			for (var i = 0; i < carNumAll.length; i++) {
			$('.list-cl').append('<li class="col-xs-6"><span>'+carNumAll[i].carnum+'</span></li>');	
			}
		}
	})
}
